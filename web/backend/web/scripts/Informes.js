var Informes = {
  init: function () {
    ...
    if (elemento) doubleScroll(elemento);

    $("#tabla-informe").stickyTableHeaders({
        ...
        generarInforme: function (idReporte) {
          ...
          request.done(function (data) {
            if (!data.error) {
              ...
              var verificarEstado = function (tiempo) {
                setTimeout(function () {
                  $.get("/informes/estado/" + idReporte, { key: key }).done(
                    function (data) {
                      if (data.ready)
                        window.location =
                          "/informes/" + idReporte + "?key=" + key;
                      else verificarEstado(tiempo < 5 ? tiempo + 1 : tiempo);
                    }
                  );
                }, tiempo * 1000);
              };
          ...
          request.fail(function () {
            ...
            Vue.set(_this, "error", {
              tipo: "danger",
              texto:
                "Ocurrió un error realizando la operación. Contacte con el administrador.",
            });
          });
        },
              ...
              var verificarEstadoPartido = function (tiempo) {
                setTimeout(function () {
                  ...
                  requestEstado.done(function (data) {
                    if (data.ready) {
                      $.get("/informes/excel-partido/", { key: key })
                        .done(function (data) {
                          var verificarEstadoTabla = function (tiempo) {
                            setTimeout(function () {
                              $.get("/informes/estado-tabla/", { key: key })
                                .done(function (data) {
                                  if (data.ready) {
                                    _this.cargando = false;
                                    window.location.assign(
                                      "/informes/descargar?key=" +
                                        key +
                                        "&nombre=" +
                                        nombre
                                    );
                                  } else verificarEstadoTabla(tiempo < 5 ? tiempo + 1 : tiempo);
                                })
                                .fail(function () {
                                  _this.cargando = false;
                                });
