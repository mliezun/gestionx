var Main = {
      ...
      .load(url, function () {
        ...
        setTimeout(function () {
          ...
          if ($primerInput.hasClass("select2-hidden-accessible"))
            ...
            ($primerInput.attr("type") == "text" ||
              $primerInput.is("select")) &&
            // es datepicker
            !$primerInput.hasClass("datepicker-to") &&
            !$primerInput.hasClass("datepicker-from") &&
            !$primerInput.parent().hasClass("date")
          )
            $primerInput.focus();
  ...
  imprimir: function () {
    ...
    if (!colapsado) $("body").addClass("sidebar-collapse");

    $("body").css("zoom", "0.5");
    window.print();
    $("body").css("zoom", "1");

    if (!colapsado) $("body").removeClass("sidebar-collapse");
  },
  ...
  _restaurarMask: function (inputs, maskOptions) {
    for (var i = 0; i < inputs.length; i++) {
      if (maskOptions[i]) $(inputs[i]).inputmask(maskOptions[i]);
    }
  },
};
